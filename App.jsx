  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {detailItem && !editing ? (
        ...
        /* ── List view (title + add button moved to the bottom, below the list) ── */
        <>
          {timeline.map((t,i)=>(
          ...
          <Card style={{ marginTop:4 }}>
            ...
            <div style={{ fontSize:12, color:C.gray }}>ดูประวัติการรักษาทั้งหมด {timeline.length} รายการด้านบน</div>
          </Card>
        </>
      ) : null}
    </div>
...
const STORAGE_KEY = "cat-health-tracker-data";
...
export default function App() {
  ...
  const [saveState, setSaveState]=useState("loading"); // loading | saved | saving | error
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // no saved data yet — that's fine, keep defaults
        setSaveState("saved");
      }
      setLoaded(true);
    })();
  ...
  // ── Save data whenever anything changes (after initial load) ──
  useEffect(() => {
    if (!loaded) return;
    setSaveState("saving");
  ...
  }, [cat, weights, timeline, meds, water, food, hospitals, stock, checklist, loaded]);
  ...
  return (
    <div style={{ background:C.cream, minHeight:"100vh", fontFamily:"'Noto Sans Thai','Sarabun',-apple-system,sans-serif", maxWidth:480, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      {/* Top bar */}
      <div style={{ position:"sticky", top:0, zIndex:200, background:"rgba(255,248,240,0.95)", backdropFilter:"blur(12px)", borderBottom:`1px solid #F3F4F6`, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:C.gray }}>Health Tracker · บันทึกในเครื่องนี้</div>
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ background:(saveState==="error"?C.red:C.green)+"22", color:saveState==="error"?C.red:C.green, borderRadius:99, padding:"4px 10px", fontSize:11, fontWeight:600 }}>
            {saveState==="saving" ? "⏳ กำลังบันทึก..." : saveState==="error" ? "⚠️ บันทึกไม่สำเร็จ" : "💾 บันทึกอัตโนมัติแล้ว"}
          </div>
      ...
      {/* Content */}
      <div style={{ padding:"16px 16px 16px", flex:1, display:"flex", flexDirection:"column", minHeight:0 }}>
